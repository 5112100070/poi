...
if ($portal_auth_d!=1) { 
     $cek=OCIParse($connect, "SELECT * FROM LOGIN WHERE NIK='$username' AND PASSWORD='$password'");
     OCIExecute($cek);
     ...
     $cek=OCIParse($connect, "SELECT * FROM LOGIN WHERE NIK='$username'");
     OCIExecute($cek);
     $hasil=OCI_Fetch_array($cek);
